...
2. Send a payment
    ...
    In the following example, replace `{ACCOUNT_ADDRESS_HERE}` with your Address (2 locations), `{API_KEY_HERE}` with your API key, and `{DESTINATION_ADDRESS_HERE}` with a destination address. If you are using the Test Net, you can use the address `rLisz7Qd3hQLuJcHn7TpsHF5KuExyr41c8` as a destination.
